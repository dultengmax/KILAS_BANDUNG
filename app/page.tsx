...
import { HeroCarousel, HeroCarousel2 } from "@/components/hero-carousel"
import Times from "@/components/time"
import { getArticles, searchArticles } from "@/lib/action/article"
		...
		<>
			...
			<main className="min-h-screen bg-background dark:bg-background smooth-transition">
				...
				<section className="w-full md:w-7xl mx-auto justify-center gap-2 grid grid-cols-1 md:grid-cols-4 md:mt-3">
					...
					>
						<HeroCarousel2 sideHeroes={sideHeroes} />
					</Suspense>
				</section>
					...
					<div className="grid grid-cols-2 max-w-7xl mx-auto pt-3 md:grid-cols-3 mt-7 gap-5">
						{news?.map((article: any, idx: number) => (
							<Link
								...
								className="group flex flex-col justify-end bg-card dark:bg-card hover:shadow-lg dark:hover:shadow-xl transition-all duration-300 rounded-lg overflow-hidden border border-border dark:border-border hover:border-primary/50 dark:hover:border-secondary/50 smooth-transition relative"
							...
							>
								...
								<div className="absolute inset-0 w-full h-full z-0">
									<Image
										...
										priority={idx < 6}
									/>
									{/* Overlay for better text readability with visible gradient */}
									<div className="absolute inset-0 z-10 pointer-events-none">
										{/* Bottom to top gradient */}
										<div className="absolute inset-0 bg-linear-to-t from-black/85 via-black/40 to-transparent" />
										{/* Optional: subtle side linear for extra polish */}
										<div className="absolute inset-0 bg-linear-to-l from-black/20 via-transparent to-transparent" />
									</div>
								</div>
								{/* Content above image */}
								<div className="relative z-20 flex-1 p-4 flex flex-col h-full justify-end">
									<span className="text-xs font-bold text-primary dark:text-secondary mb-2 uppercase drop-shadow-[0_1px_2px_rgba(0,0,0,.7)]">
										{categoryName.find((item: any) => item.id === Number(article.category[0]))?.name}
									</span>
									<h3 className="text-lg font-bold text-white mb-2 line-clamp-2 group-hover:text-primary dark:group-hover:text-secondary transition-colors drop-shadow-[0_2px_4px_rgba(0,0,0,.6)]">
										{article.title}
									</h3>
									<p className="text-sm text-white/90 mb-4 line-clamp-2 flex-1 drop-shadow-[0_1px_2px_rgba(0,0,0,.7)]">
										{article.excerpt}
									</p>
									<div className="flex justify-between items-center text-xs text-white/80 drop-shadow-[0_1px_2px_rgba(0,0,0,.8)]">
					...
					<div className="grid grid-cols-1 mx-auto lg:grid-cols-3 gap-8 w-full max-w-7xl">
						...
						<div className="lg:col-span-2">
							...
							<section className="mb-16 pb-16 border-b border-border dark:border-border">
								<div className="flex items-center gap-4 mb-10 px-6 py-4 rounded-xl bg-linear-to-r from-blue-600 via-blue-400 to-sky-400 dark:bg-linear-to-br dark:from-blue-900 dark:via-blue-700 dark:to-blue-500 shadow-lg">
									<div className="w-1.5 h-10 rounded-full bg-linear-to-b from-amber-400 via-orange-500 to-pink-500 shadow-md" />
								...
								<Suspense fallback={
									<div className="grid grid-cols-2 mx-auto max-w-7xl md:grid-cols-3 gap-6">
										{Array.from({ length: 6 }).map((_, idx) => (
											<div
												key={idx}
												className="animate-pulse flex flex-col bg-card dark:bg-card rounded-lg overflow-hidden border border-border dark:border-border"
